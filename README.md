...
│   │                                            # and updated to OpenTelemetry 1.19.0.
...
| Client `OpenTelemetry.*` | `1.19.0` | nuget.org |
...
The **ClientServiceDefaults** project tracks the `dotnet new blazorwasm-servicedefaults` template
shipped in the RC1 SDK, with two intentional differences: it uses OpenTelemetry 1.19.0 so the
template's browser OTLP export workaround can be removed, and it uses a 5s metric export interval
instead of the SDK default of 60s so client metrics appear on the dashboard while on stage.
...
- **Explicit `v1/logs` / `v1/traces` / `v1/metrics` endpoints resolved against
  ...
  template.
- **`NullLogger` shim in place of the app's `ILoggerFactory`** — removed when the template switched
  to lazy logger resolution inside `BackgroundExportHandler`.
- **`BackgroundExportHandler` and its custom Polly retry pipeline** — removed after upgrading the
  OpenTelemetry packages to 1.19.0. The OTLP exporter now disables `IHttpClientFactory` integration
  on browser platforms by default, avoiding the sync-over-async deadlock tracked by
  [open-telemetry/opentelemetry-dotnet#7708](https://github.com/open-telemetry/opentelemetry-dotnet/issues/7708)
  without a template-level fire-and-forget handler. Verified end-to-end in Edge: the app boots with
  no console errors, the Weather page loads 5 forecast rows through service discovery, all three
  OTLP endpoints (`v1/logs`, `v1/traces`, `v1/metrics`) return 200, and the dashboard shows the
  correlated `app` → `gateway` → `apiservice` trace.
...
- Browser OTLP deadlock workaround: [`open-telemetry/opentelemetry-dotnet#7709`](https://github.com/open-telemetry/opentelemetry-dotnet/pull/7709)
